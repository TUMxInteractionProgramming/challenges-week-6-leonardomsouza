
var dummyMessage1 = {
    expiresOn:Date.now() + 1000*60*15,
    createdBy: "teste.menssagem.burra",
    createdOn:Date.now(),
    expiresIn:0,
    text: "texto burro 1",
};


var dummyMessage2 = {
    expiresOn:Date.now() + 1000*60*15,
    createdBy: "teste.menssagem.burra",
    createdOn:Date.now(),
    expiresIn:0,
    text: "texto burro 2",
};


var dummyMessage3 = {
    expiresOn:Date.now() + 1000*60*15,
    createdBy: "teste.menssagem.burra",
    createdOn:Date.now(),
    expiresIn:0,
    text: "texto burro 3",
};



var yummy = {
    name: "#Yummy",
    createdOn: new Date(2016, 03, 01), /* month 0 is jan. */
    createdBy: "minus.plus.yummy",
    starred: false,
    expiresIn: 100,
    messageCount: 999,
    messages : [dummyMessage1, dummyMessage2]           // #10 #message s property in #chl object
};

var sevencontinents = {
    name: "#SevenContinents",
    createdOn: new Date(2016, 03, 02), /* month 0 is jan. */
    createdBy: "cheeses.yard.applies",
    starred: true,
    expiresIn: 60,
    messageCount: 5,
    messages : [dummyMessage1]           // #10 #message s property in #chl object

};

var killerapp = {
    name: "#KillerApp",
    createdOn: new Date(2016, 08, 30), /* month 0 is jan. */
    createdBy: "lodge.bits.fake",
    starred: false,
    expiresIn: 1,
    messageCount: 10351,
    messages : [dummyMessage1, dummyMessage3]           // #10 #message s property in #chl object

};

var firstpersononmars = {
    name: "#FirstPersonOnMars",
    createdOn: new Date(2016, 08, 28), /* month 0 is jan. */
    createdBy: "snipped.atom.grid",
    starred: true,
    expiresIn: 30003,
    messageCount: 2424,
    messages : [dummyMessage2, dummyMessage1, dummyMessage3]           // #10 #message s property in #chl object

};

var octoberfest = {
    name: "#Octoberfest",
    createdOn: new Date(2016, 08, 25), /* month 0 is jan. */
    createdBy: "vocally.clearly.crawled",
    starred: false,
    expiresIn: 60,
    messageCount: 321,
    messages : [dummyMessage2, dummyMessage3]           // #10 #message s property in #chl object

};

